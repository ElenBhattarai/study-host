...
export default function JoinSession(props) {
    ...
    async function fetchSessions(){
        const response = await fetch('https://studyhostbackend.duckdns.org/allsessions')
        const data = await response.json()
        setAll(data.sessions)
        setSessions(data.sessions)
        const res = await fetch(`https://studyhostbackend.duckdns.org/joinedsession?user_id=${sessionStorage.getItem('user_id')}`)
        ...
        let arr2 = []
        for(let i of arr){
            const res = await fetch(`https://studyhostbackend.duckdns.org/getsession?sess=${i}`)
            const data = await res.json()
            arr2.push(data.sessions[0])
        }
        setMy(arr2)
        const now = await fetch('https://studyhostbackend.duckdns.org/happeningnow')
    ...
    const viewParticipants = async(session) => {
        setSession(session)
        const res = await fetch(`https://studyhostbackend.duckdns.org/getparticipants?session_id=${session.session_id}`)
        const data = await res.json()
        setParticipants(data)
        console.log(participants)
        const res1 = await fetch(`https://studyhostbackend.duckdns.org/user?user_id=${session.user_id}`)
    ...
    const handleJoin = async() => {
        const requestOptions = {
          ...
          try {
            const response = await fetch('https://studyhostbackend.duckdns.org/joinsession', requestOptions);
          } catch(err) {
            console.log(err)
          }
        setIntro('')
