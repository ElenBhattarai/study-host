...
export default function CreateSession() {
  ...
  async function fetchCourses() {
    const response = await fetch('https://studyhostbackend.duckdns.org/getdepts');
  ...
  async function handleDepartmentChange(event) {
    ...
    const response = await fetch(`https://studyhostbackend.duckdns.org/getcourses?dept=${selectedDepartment}`);
    const data = await response.json();
  ...
  const handleSubmit = async() => {
    const requestOptions = {
      ...
      try {
        await fetch('https://studyhostbackend.duckdns.org/createsession', requestOptions);
